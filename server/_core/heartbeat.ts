...
const buildEndpoint = (rpc: string): string => {
  if (!ENV.schedulerApiUrl) {
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
      message: "Heartbeat service URL is not configured (SCHEDULER_API_URL).",
    });
  }
  if (!ENV.schedulerApiKey) {
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
      message: "Heartbeat service API key is not configured (SCHEDULER_API_KEY).",
    });
  }
  const baseUrl = ENV.schedulerApiUrl;
...
const callScheduler = async <T>(
  ...
  const headers: Record<string, string> = {
    ...
    authorization: `Bearer ${ENV.schedulerApiKey}`,
 ...
 * owner-only via the scheduler provider's administrative console.
 */
